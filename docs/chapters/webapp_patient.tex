\chapter{Веб-приложение пациента}\label{ch:-211}

В эту часть веб приложения можно попасть из той же страницы авторизации, но только при использовании данных авторизации пациента.
В приложении реализована система ролей, состоящая из двух ролей: врач и пациент.
В роутере (react router dom) сделана ленивая загрузка (lazy loading) так, что в начальный момент времени загружаются только страницы авторизации и регистрации.
Все остальные подгружаются с сервера только после успешной авторизации, и то только те страницы, которые используются данной ролью.
Это позволяет сократить вес приложения, которое загружается с сервера.
То есть ускоряет загрузку, а также скрывает код приложения неавторизованным пользователям, что обеспечивает дополнительную безопасность.


Веб-приложение пациента повторяет функционал Android-приложения.

\section{Главная страница}\label{sec:-212}
...
\section{Прохождения опроса}\label{sec:-213}
...
\section{PUSH-уведомления}\label{subsec:-215}
Через нужный промежуток времени пациенту приходит уведомление о необходимости прохождения опроса.
При клике на него, пациент переходит на нужный опрос.
(Cм.\ рис. \ref{fig:figure59})

\begin{figure}[htbp]
    \centering
    \includegraphics[width=\dimexpr\textwidth/3\relax]{images/notification}
    \caption{Пример уведомления для пациента.}
    \label{fig:figure59}
\end{figure}
